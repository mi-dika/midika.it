...
import { fetchCookiePolicy } from '@/lib/iubenda';

export const metadata: Metadata = {
  title: 'Cookie Policy',
  description:
    'Cookie Policy of MiDika - Italian software house focused on minimalism and design.',
...
export default async function CookiePolicyPage({ searchParams }: PageProps) {
  ...
  try {
    policyContent = await fetchCookiePolicy(language);
  } catch (error) {
    console.error('Failed to fetch cookie policy:', error);
    policyContent =
      '<p>Unable to load cookie policy at this time. Please try again later.</p>';
  }
